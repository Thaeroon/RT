...
/*   Updated: 2018/04/26 22:50:24 by afertah          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		hit_middle(t_object *object, t_ray r, t_var var, t_hit_rec *rec)
{
	if (var.t0 <= 0)
		return (0);
...
int		hit_obj(t_ray r, t_object *object, t_var v, t_hit_rec *rec)
{
	if (v.axe0 < -object->cut_lvl)
	{
		...
		{
			v.th = v.t0 + (v.t1 - v.t0) * (v.axe0 + object->cut_lvl) / (v.axe0 - v.axe1);
			return (hit_top(r, v, rec));
		}
	}
	else if (v.axe0 >= -object->cut_lvl && v.axe0 <= object->cut_lvl)
		return (hit_middle(object, r, v, rec));
	else if (v.axe0 >= object->cut_lvl)
	{
		if (v.axe1 >= object->cut_lvl)
			return (0);
		else
